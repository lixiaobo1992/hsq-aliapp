...
const mutations = {
  ...
  ['SET_ADDRESSES'](state, address) {
    ...
    console.log({abc: state.addresses})
  },
  ...
  return new Promise(function (resolve, reject) {
    api.addressList({}, (res) => {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        var address = result.province + result.city + result.district + result.detail_address;
        if (result.province == result.city) {
          address = result.city + result.district + result.detail_address;
        }
        resultNew.currentProvinceId = results[i].province_id;
